# -*- coding: utf-8 -*-
"""
Created on Sat Feb 20 08:36:22 2021
...
#################### IMPLEMENTATION DES FONCTIONS DE COÛT ###################
...
def descente_gradient(datax, datay, f_loss, f_grad, eps, maxIter, descent='batch', mb = 10):
    ...
    w = np.random.rand(datax.shape[1]).reshape(1,-1)
    allw = [ w.tolist() ]                            # tous les w calculés
    ...
    for niter in range(maxIter):
        ...
        if descent == 'stochastique':
            ...
            for j in range(len(datax)):
                ...
                y = np.array([ datay[i] ]).reshape(1,-1)
                w -= eps * f_grad( w.reshape(1,-1), x, y).reshape(datax.shape[1],)
        if descent == 'mini-batch':
            for j in range(len(datax)//mb):
                inds = [rd.randint(0,len(datax)-1) for i in range(mb)]
                delta = np.mean( [ f_grad( w.reshape(1,-1), datax[i].reshape(1,-1), np.array([ datay[i]]).reshape(1,-1)) for i in inds], axis = 0 )
                w -= eps * delta.reshape(datax.shape[1],)
            
        # Mise à jour de allw et allf
        allw.append( w.tolist() )
